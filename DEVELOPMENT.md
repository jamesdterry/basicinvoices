...
- **PDF**: `pdfkit` (pure JS, no native deps). Originally `puppeteer-core` + `@sparticuz/chromium` for HTML/CSS-rendered visual parity with the web view, but the chrome-headless-shell binary kept hitting a "Requesting main frame too early" race on Fly's slim image even with retries — swapped to pdfkit and a hand-drawn layout. Numbers/lines are identical to the public `/i/<token>` page; the visual layout is its own thing.
